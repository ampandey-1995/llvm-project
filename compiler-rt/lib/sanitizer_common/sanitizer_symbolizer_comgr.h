//===-- sanitizer_symbolizer_comgr.h ----------------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// Subset of the AMD Code Object Manager (Comgr) ABI used to symbolize AMDGPU
// code objects, plus the table of entry points resolved out of
// libamd_comgr.so at runtime.
//
// __UINT64_TYPE__ and __SIZE_TYPE__ are what uint64_t and size_t resolve to,
// spelled so this header needs no includes at all. Include it only from
// SANITIZER_AMDHSA-guarded code, as the HSA headers are.
//
//===----------------------------------------------------------------------===//

#ifndef SANITIZER_SYMBOLIZER_COMGR_H
#define SANITIZER_SYMBOLIZER_COMGR_H
...
#endif  // SANITIZER_SYMBOLIZER_COMGR_H
